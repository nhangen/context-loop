...
The divisor is a **dynamic variable**. `CONTEXT_LOOP_WINDOW`, if set to a positive integer, is used verbatim — this is the authoritative override for sessions whose real window the hook can't infer (1M-beta `[1m]`, fast-mode, model aliases, future models). With no override, it falls back to a model-name heuristic: `opus`, `fable`, `mythos`, `sonnet-5`, and `sonnet-4-6` → 1,000,000; everything else (including `sonnet-4-5` and earlier) → 200,000. The match is on the version-tagged substring, not the bare family name — model ids embed the generation precisely (`claude-sonnet-5`, `claude-sonnet-4-6`, `claude-sonnet-4-5-<date>`), so this tells a 1M Sonnet apart from a 200K one without guessing.
